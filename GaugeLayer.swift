...
//  Updated to Swift 3 by Omar Albeik https://github.com/omaralbeik
...
class GaugeLayer: CALayer {
	...
	override init(layer: Any) {
		...
		if let gaugeLayer = layer as? GaugeLayer {
			startAngle = gaugeLayer.startAngle
			stopAngle = gaugeLayer.stopAngle
			
			radius = gaugeLayer.radius
			thickness = gaugeLayer.thickness
			gaugeBackgroundColor = gaugeLayer.gaugeBackgroundColor
			gaugeColor = gaugeLayer.gaugeColor
		}
		setup()
	}
	...
	private func drawGauge(startPoint: CGPoint) -> CGMutablePath {
		...
		path.addArc(center: center, radius: radius, startAngle: CGFloat(startAngle), endAngle: CGFloat(stopAngle), clockwise: false)
		...
		path.addArc(center: center, radius: radius - thickness, startAngle: CGFloat(stopAngle), endAngle: CGFloat(startAngle), clockwise: true)
		return path
	}
	...
	override func draw(in ctx: CGContext) {
		...
		let point1 = CGPoint(x: frame.size.width/2, y: frame.size.height/2)
		ctx.addArc(center: point1, radius: radius - (thickness/2), startAngle: 0, endAngle: CGFloat(2.0*M_PI), clockwise: false)
